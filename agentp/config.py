...
@dataclass
class LLMConfig:
    provider: str = _env("AGENTP_PROVIDER", "mock")  # mock | openai
    model: str = _env("AGENTP_MODEL", "mock-medium")
    api_key: str = _env("AGENTP_API_KEY", "")
    base_url: str = _env("AGENTP_BASE_URL", "https://api.deepseek.com/v1")
    embedding_model: str = _env("AGENTP_EMBEDDING_MODEL", "text-embedding-3-small")
    temperature: float = _env("AGENTP_TEMPERATURE", 0.3, float)
    timeout_s: float = _env("AGENTP_TIMEOUT", 60.0, float)
    max_retries: int = _env("AGENTP_MAX_RETRIES", 2, int)
...
class ContextConfig:
    ...
    model_context_window: int = _env("AGENTP_CTX_WINDOW", 8192, int)
    reserve_for_output: int = _env("AGENTP_RESERVE_OUTPUT", 1024, int)
...
class LoopConfig:
    ...
    max_steps: int = _env("AGENTP_MAX_STEPS", 12, int)
    max_tokens: int = _env("AGENTP_MAX_TOKENS", 60000, int)
    max_cost_usd: float = _env("AGENTP_MAX_COST", 0.5, float)
    max_wall_time_s: float = _env("AGENTP_MAX_WALL", 120.0, float)
